...
static void supervisor(int seccomp_notify_fd, void *ctx, seccomp_unotify_handler *syscall_handlers)
{
	...
	while (1)
	{
		...
		if (poll_seccomp_notify_fd.revents & POLLHUP) break;
...
static volatile int target_exit_code;

static void on_sigchld(int, siginfo_t *info, void *)
{
	if (info->si_pid == target_pid)
	{
		if (info->si_code == CLD_EXITED) target_exit_code = info->si_status;
		else target_exit_code = -1;

		debug_printf("target %d exited with status %d", target_pid, target_exit_code);
...
int seccomp_unotify_vfork_exec(void *ctx, seccomp_unotify_handler *syscall_handlers, size_t len_syscall_handlers, char *file, char **argv, char **envp)
{
	...
	supervisor(seccomp_notify_fd, ctx, syscall_handlers);
	close(seccomp_notify_fd);
